...
Directory structure on the Jetson (auto-created on first run):
    ~/kd_ai/
        ...
        piper/piper                       <- Piper TTS binary (installed by setup_phase2.sh)
        logs/                             <- Daily log files
        assets/
            voices/                       <- Piper voice model files (.onnx)
            backgrounds/                  <- Background video loops (.mp4) for Phase 2
        content/
            on_demand/                    <- Output from forge.py
            autonomous/
                queue/                    <- Generated, awaiting Kian review
                approved/                 <- Approved, ready for video production
                rejected/                 <- Rejected, archived
                produced/                 <- Finished .mp4 files ready for upload
...
AUTONOMOUS_PRODUCED_DIR = AUTONOMOUS_DIR / "produced"
LOG_DIR                 = KD_AI_ROOT / "logs"
SYSTEM_PROMPT_FILE      = KD_AI_ROOT / "system_prompt.txt"
ASSETS_DIR              = KD_AI_ROOT / "assets"
